...
function AnimatedSection({ children, className = "" }: { children: ReactNode; className?: string }) {
  ...
  return <motion.div ref={ref} variants={stagger} initial={prefersReducedMotion ? "visible" : "hidden"} animate={animateState} className={className}>{children}</motion.div>;
}
...
function Hero({ copy }: { copy: PageCopy }) {
  ...
  const enableMotionEffects = !prefersReducedMotion && !isMobile;
  ...
  return (
    <section ref={heroRef} id="hero" className="relative flex min-h-screen items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        <motion.img src={HERO_BG} alt="" style={enableMotionEffects ? { y: imageY, scale: imageScale } : undefined} className="h-full w-full object-cover opacity-45" />
        ...
        {enableMotionEffects && <motion.div style={{ y: glowY }} className="ambient-orb ambient-orb-gold left-[8%] top-[18%] h-64 w-64 opacity-80" />}
        {enableMotionEffects && <motion.div style={{ y: glowY }} className="ambient-orb ambient-orb-soft bottom-[12%] right-[8%] h-72 w-72 opacity-70" />}
        <div className="section-grid absolute inset-0 hidden opacity-40 md:block" />
        <div className="noise-overlay absolute inset-0 hidden md:block" />
      </div>

      <div className="relative container pb-16 pt-28 md:pb-24 md:pt-36">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="section-label mb-6">
          {copy.hero.eyebrow}
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }} className="font-display mb-6 max-w-5xl text-[2.7rem] font-black leading-[0.94] sm:text-6xl lg:text-[6.25rem] xl:text-[7.25rem]">
          <span className="block whitespace-nowrap text-white">{copy.hero.titleTop}</span>
          <span className="gold-shimmer block whitespace-nowrap">{copy.hero.titleBottom}</span>
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 }} className="mb-10 max-w-2xl text-base leading-relaxed text-white/75 sm:text-lg md:max-w-3xl md:text-2xl">
          {copy.hero.description}
        </motion.p>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="mb-12 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
          <a href="#portfolio" className="inline-flex w-full items-center justify-center gap-2 rounded-sm bg-gold px-6 py-3 text-xs font-bold uppercase tracking-wider text-background transition-all hover:bg-gold-light hover:shadow-lg hover:shadow-gold/20 sm:w-auto sm:px-10 sm:py-4 sm:text-sm md:text-base">
            <Sparkles size={18} />
            {copy.hero.primaryCta}
          </a>
          <a href="#contact" className="inline-flex w-full items-center justify-center gap-2 rounded-sm border border-gold/30 px-6 py-3 text-xs font-bold uppercase tracking-wider text-gold transition-all hover:bg-gold/10 sm:w-auto sm:px-10 sm:py-4 sm:text-sm md:text-base">
            {copy.hero.secondaryCta}
          </a>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.7 }} className="flex flex-wrap gap-x-5 gap-y-3 sm:gap-7">
          {copy.hero.highlights.map((item) => (
            <div key={item} className="flex items-center gap-2">
              <div className="h-1.5 w-1.5 rounded-full bg-gold" />
              <span className="font-display text-[11px] font-bold uppercase tracking-[0.22em] text-white/55 sm:text-sm sm:tracking-widest">{item}</span>
            </div>
          ))}
        </motion.div>
      </div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="absolute bottom-8 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 text-white/30 md:flex">
    ...
    <section id="services" className="relative overflow-hidden py-24 md:py-32">
      ...
      <div className="relative container">
        <AnimatedSection className="mb-16 text-center">
          ...
          <motion.h2 variants={fadeUp} className="font-display mb-4 text-3xl font-black text-white sm:text-4xl md:text-6xl">{copy.servicesSection.title} <span className="text-gold">{copy.servicesSection.accent}</span></motion.h2>
      ...
      <div className="container">
        <div className="grid items-start gap-16 lg:grid-cols-2">
          <AnimatedSection>
            ...
            <motion.h2 variants={fadeUp} className="font-display mb-6 text-4xl font-black text-white sm:text-5xl md:text-6xl">{copy.contact.titleTop}<br /><span className="text-gold">{copy.contact.titleBottom}</span></motion.h2>
