/* ============================================================
   ...
   Design: DatiVe Design | Development: DatiVe Design
...
import {
  ...
  Layers, Pen, Sparkles, Linkedin, X
} from "lucide-react";
...
// ─── Portfolio Projects KWADRATY Z PRACA ────────────────────────────────────────────────────────
const projects = [
  {
    client: "Free Premium Beer",
    title: "NOWA ETYKIETA, NOWA JAKOŚĆ",
    category: "LABEL DESIGN",
    description: "Projekt etykiety piwa premium skupiony na nowoczesnej estetyce i czytelności.",
    tags: ["#LabelDesign", "#PackagingDesign", "#BeerDesign"],
    reactions: "2",
    image: PACKAGING_IMG, // Główne zdjęcie (okładka w siatce)
    // DODAJEMY TO: Lista zdjęć w środku modala
    gallery: [PACKAGING_IMG, PORTFOLIO_SHOWCASE, BRANDING_IMG], 
  },
  {
    client: "Bean to Bite",
    title: "Elegancja z Nutą Natury",
    category: "Post Reklamowy DatiVe Design",
    description: "Spójny zestaw materiałów: Giftcards, ulotki i wizytówki oddające charakter marki.",
    tags: ["#Branding", "#IdentityDesign"],
    reactions: "4 reakcje",
    image: PORTFOLIO_SHOWCASE,
  },
  {
    ...
    category: "BEAN TO BITE",
    description: "Logo w kilku wariantach kolorystycznych — od złota na czerni po monochrom.",
    tags: ["#LogoDesign", "#Branding"],
    reactions: "3 reakcje",
    image: BRANDING_IMG,
  },
  {
    ...
    category: "ChunkServe.pl",
    description: "Grafiki promujące wakacyjne rabaty łączące klimat lata z estetyką Minecraft.",
    tags: ["#GrafikaReklamowa", "#Gaming"],
    reactions: "1 reakcja",
    image: SERVICES_BG,
  },
  {
    ...
    category: "Wul-Kam",
    description: "Projekt materiałów reklamowych dla serwisu opon, stawiający na profesjonalny wizerunek.",
    tags: ["#UIUX", "#IdentyfikacjaWizualna"],
    reactions: "5 reakcji",
    image: "/portfolio5.jpg", // Tutaj dodany plik portfolio5
  },
  {
    client: "Phoenix",
    title: "Billboard z Charakterem",
    category: "Phoenix",
    description: "Czysta i nowoczesna identyfikacja wizualna dla studia wrappingu samochodowego",
    tags: ["#Minimalism", "#Design"],
    reactions: "6 reakcji",
    image: "/portfolio6.jpg", // Tutaj dodany plik portfolio6
  },
];
...
<a href="#hero" className="flex items-center group">
          {/* Zmieniłem h-8 na h-10, żeby samo logo było troszkę większe i lepiej widoczne */}
          <img src="/logo.png" alt="Dative Design logo" className="h-13 w-auto" />
        </a>
        ...
        <motion.a 
          ...
          whileTap={{ scale: 0.98 }}
          className="hidden md:inline-flex items-center gap-2 px-6 py-2.5 gold-button-shimmer text-background font-display font-black text-sm tracking-wider uppercase rounded-sm transition-all"
        >
      ...
      <div className="relative container pt-24 pb-16">
        <div className="w-full">
          ...
          <motion.h1
            ...
            className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-9xl font-black leading-none mb-4"
          >
            <span className="block text-white">Z POMYSŁU</span>
            <span className="block gold-shimmer whitespace-nowrap">DO REALIZACJI</span>
          </motion.h1>
          ...
          animate={{ 
            boxShadow: [
              ...
              "0px 0px 20px 2px rgba(212, 175, 55, 0.6)", 
              "0px 0px 0px 0px rgba(212, 175, 55, 0)"
            ]
          }}
          transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
          className="hidden md:inline-flex items-center gap-2 px-5 py-2 bg-gold text-background font-display font-bold text-sm tracking-wider uppercase rounded-sm hover:bg-gold-light transition-colors"
        >
          <Sparkles size={14} />
          NAPISZ DO MNIE
        </motion.a>
    ...
    <section id="services" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="relative container">
        <AnimatedSection className="text-center mb-16">
          ...
          <motion.p variants={fadeUp} className="text-white/50 max-w-1xl mx-auto">
      ...
      <div className="container">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <AnimatedSection>
            ...
            <motion.div variants={stagger} className="grid grid-cols-1 sm:grid-flow-col sm:grid-rows-3 gap-x-25 gap-y-6 sm:w-fit">
              {[
...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/reactions")
      ...
      .catch(err => console.error("Błąd pobierania reakcji:", err));
  }, []);

const handleReactionClick = async (e: React.MouseEvent, title: string) => {
    e.stopPropagation(); 
    
    // 1. Sprawdzamy blokadę
    const likedProjects = JSON.parse(localStorage.getItem("likedProjects") || "[]");
    if (likedProjects.includes(title)) {
      return; 
    }

    // 2. Od razu pokazujemy lajka na ekranie, żeby klient nie czekał (płynność)
    setReactionCounts(prev => ({
      ...prev,
      [title]: (prev[title] || 0) + 1
    }));

    // 3. Wysyłamy informację do serwera
    try {
      const response = await fetch(`/api/reactions/${encodeURIComponent(title)}`, {
        method: "POST"
      });

      if (response.ok) {
        // SUKCES! Serwer potwierdził zapis. Dopiero teraz blokujemy kolejne kliknięcia
        likedProjects.push(title);
        localStorage.setItem("likedProjects", JSON.stringify(likedProjects));
      } else {
        // BŁĄD SERWERA: Cofamy wizualnego lajka, bo serwer go nie przyjął
        setReactionCounts(prev => ({
          ...prev,
          [title]: (prev[title] || 0) - 1
        }));
        console.error("Serwer odrzucił zapisanie reakcji.");
      }
    } catch (error) {
      // BRAK POŁĄCZENIA: Cofamy wizualnego lajka
      setReactionCounts(prev => ({
        ...prev,
        [title]: (prev[title] || 0) - 1
      }));
  ...
  // Blokowanie scrollowania strony pod spodem, gdy modal jest otwarty
  useEffect(() => {
  ...
  // NOWE: Nawigacja klawiaturą w Modalu (ESC + Strzałki)
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Przełączanie zdjęć strzałkami (tylko jeśli jest galeria z więcej niż 1 zdjęciem)
      if (selectedProject.gallery && selectedProject.gallery.length > 1) {
        const currentIndex = selectedProject.gallery.indexOf(activeImage);
        if (currentIndex === -1) return;

        if (e.key === "ArrowRight") {
          const nextIndex = (currentIndex + 1) % selectedProject.gallery.length;
          setActiveImage(selectedProject.gallery[nextIndex]);
        } else if (e.key === "ArrowLeft") {
          const prevIndex = (currentIndex - 1 + selectedProject.gallery.length) % selectedProject.gallery.length;
          setActiveImage(selectedProject.gallery[prevIndex]);
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section id="portfolio" className="py-24 md:py-32">
        <div className="container">
<AnimatedSection className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
            {/* Lewa strona - Nagłówek */}
            <div>
              <motion.div variants={fadeUp} className="section-label mb-4">◆ Realizacje</motion.div>
              <motion.h2 variants={fadeUp} className="font-display text-5xl md:text-6xl font-black text-white leading-none">
                WYBRANE<br /><span className="text-gold">PROJEKTY</span>
              </motion.h2>
            </div>
            
            {/* Prawa strona - Poprawiony tekst ze złotą linią */}
            <motion.div 
              variants={fadeUp} 
              className="max-w-md border-l-2 border-gold/30 pl-6 py-2 mt-6 md:mt-0"
            >
          ...
          {/* SIATKA: 3 KOLUMNY W POZIOMIE, 2 RZĘDY W PIONIE */}
  ...
  setActiveImage(project.gallery ? project.gallery[0] : project.image);
}}
                  className="group relative overflow-hidden rounded-sm border border-white/5 gold-border-hover card-hover bg-card cursor-pointer"
                >
                  {/* KWADRATOWE BOXY */}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    <div className="section-label mb-2 text-[10px]">{project.category}</div>
                    <h3 className="font-display text-lg font-bold text-white mb-2 uppercase">{project.title}</h3>
                    <div className="flex flex-wrap gap-2">
                      {project.tags.map((tag) => (
                        <span key={tag} className="text-[10px] text-gold/60 font-display">{tag}</span>
                      ))}
                    </div>
                  </div>

                  <div 
  onClick={(e) => handleReactionClick(e, project.title)}
  className="absolute top-4 right-4 px-2 py-0.5 bg-background/80 backdrop-blur-sm border border-white/10 rounded-full text-[10px] text-white/50 font-display hover:text-gold hover:border-gold/50 transition-colors z-10 cursor-pointer"
>
  ♥ {reactionCounts[project.title] !== undefined ? reactionCounts[project.title] : 0}
</div>
                </motion.div>
              </AnimatedSection>
            ))}
            ...
            {reviews.map((review, i) => (
              <AnimatedSection key={i}>
                ...
                >
                  ...
                  {/* Quote z zabezpieczonymi cudzysłowami &quot; aby nie crashowało Vite'a */}
...
<div className="flex items-center">
              <img src="/logo.png" alt="Dative Design logo" className="h-11 w-auto opacity-80 hover:opacity-100 transition-opacity" />
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <a href="kontakt@dativedesign.com" className="text-white/30 hover:text-gold transition-colors">
                <Mail size={18} />
              </a>
            </div>
          </div>
      ...
      <AnimatePresence>
        {selectedProject && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-8">
            ...
            {/* Główne okno modala - RESPONSYWNE */}
            <motion.div
              initial={{ opacity: 0, scale: 0.98, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.98, y: 20 }}
              className="relative w-full h-[100dvh] md:h-[85vh] max-w-full lg:max-w-7xl bg-card border-0 md:border border-white/10 md:rounded-sm shadow-2xl overflow-hidden z-10 flex flex-col md:flex-row"
            >
              {/* Przycisk zamknięcia - mniejszy i lepiej widoczny na mobile */}
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-3 right-3 md:top-4 md:right-4 z-20 p-2 md:p-3 bg-background/80 md:bg-background/50 backdrop-blur-md border border-white/10 rounded-sm text-white hover:text-gold hover:bg-white/5 transition-all"
              >
                <X size={20} className="md:w-6 md:h-6" />
              </button>

              {/* Lewa strona - Galeria - (40% wys. na mobile, 60% szer. na desktop) */}
              <div className="w-full h-[40%] md:h-full md:w-[60%] flex flex-col bg-black/40 relative border-b md:border-b-0 md:border-r border-white/5 shrink-0">
                ...
                <div className="flex-1 relative overflow-hidden">
                  <motion.img 
                    ...
                    className="absolute inset-0 w-full h-full object-contain p-2 md:p-12"
                  />
                </div>

                {/* Miniaturki na dole - mniejsze na telefonie */}
                {selectedProject.gallery && selectedProject.gallery.length > 1 && (
                  <div className="flex gap-2 md:gap-4 p-3 md:p-6 bg-card/80 backdrop-blur-md overflow-x-auto custom-scrollbar border-t border-white/5">
                    {selectedProject.gallery.map((img, idx) => (
                      <button 
                        key={idx} 
                        onClick={() => setActiveImage(img)}
                        className={`relative shrink-0 w-16 h-16 md:w-28 md:h-28 rounded-sm overflow-hidden transition-all duration-300 ${
                          activeImage === img 
                            ? 'ring-2 ring-gold scale-95 opacity-100' 
              ...
              {/* Treść - Prawa strona - Scrollowana na mobile */}
              <div className="w-full h-[60%] md:h-full md:w-[40%] p-6 md:p-14 flex flex-col overflow-y-auto bg-card">
                <div className="section-label mb-2 md:mb-4 text-xs md:text-sm">{selectedProject.category}</div>
                <h3 className="font-display text-2xl md:text-5xl font-black text-white mb-4 md:mb-8 uppercase leading-tight">
                  {selectedProject.title}
                </h3>
                
                <div className="gold-line w-16 md:w-24 mb-6 md:mb-8 shrink-0" />
                
                <p className="text-white/70 text-sm md:text-lg leading-relaxed mb-8 md:mb-12 font-light">
                  {selectedProject.description}
                </p>

                <div className="mt-auto">
                  <div className="text-xs md:text-sm font-display font-bold tracking-wider text-white/40 uppercase mb-3 md:mb-4">Tagi projektu:</div>
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    {selectedProject.tags.map((tag) => (
                      <span key={tag} className="text-xs md:text-sm px-3 py-1.5 md:px-4 md:py-2 bg-white/5 border border-white/5 text-gold/80 font-display tracking-wider rounded-sm">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                {/* Złoty przycisk CTA - przeskalowany pod telefony */}
                <div className="mt-8 md:mt-10 pt-6 md:pt-8 border-t border-white/10 shrink-0">
                  <a
                    href="#contact"
                    onClick={(e) => {
                      e.preventDefault();
                    ...
                    className="inline-flex items-center justify-center gap-2 md:gap-3 w-full px-4 py-3 md:px-8 md:py-4 bg-gold text-background font-display font-black text-xs md:text-sm tracking-widest uppercase rounded-sm hover:bg-gold-light transition-all hover:shadow-[0_0_20px_rgba(212,175,55,0.3)] group"
                  >
                    <Sparkles size={16} className="transition-transform group-hover:scale-125" />
