  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/5 bg-background/85 backdrop-blur-xl">
      <div className="container flex h-18 items-center justify-between gap-4 py-3">
        <Link href={getStaticPath("home")} className="flex items-center gap-3">
          <img src="/logo.png" alt="DatiVe Design" className="h-12 w-auto" />
        </Link>
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-sm border border-white/10 p-3 text-white lg:hidden"
          aria-label={
            menuOpen
              ? locale === "en"
                ? "Close menu"
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`font-display text-sm font-bold uppercase tracking-wide ${
                  item.key === "order"
                    ? "rounded-full border border-gold/25 px-4 py-3 text-gold"
                    : isActive(item.key)
                      ? "text-gold"
                      : "text-white/75"
                }`}
