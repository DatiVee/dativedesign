...
// =============================================================================
// NOWOŚĆ: Nasze API do obsługi lajków
// =============================================================================
function vitePluginReactionsAPI(): Plugin {
  const DB_FILE = path.join(PROJECT_ROOT, "reactions.json");
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use(async (req, res, next) => {
        ...
        // 2. Dodawanie lajków (wyciągamy tytuł z URL: /api/reactions/TytulProjektu)
        if (req.url?.startsWith("/api/reactions/") && req.method === "POST") {
          const titleRaw = req.url.replace("/api/reactions/", "");
          ...
          try {
            ...
            fs.writeFileSync(DB_FILE, JSON.stringify(reactions, null, 2));
        ...
        // Jeśli to nie jest nasze API, puść żądanie dalej
        next();
      });
    },
  };
...
const plugins = [
  ...
  vitePluginReactionsAPI() // <--- Dodaliśmy naszą wtyczkę!
];
